...
std::array<double, 36> MakeDiagonalCovariance(
  ...
  const auto& values = diagonal.size() == fallback.size()
      ? diagonal
      : std::vector<double>(fallback.begin(), fallback.end());

  if (diagonal.size() != fallback.size()) {
    RCLCPP_WARN(
        logger,
        "Parameter '%s' should contain 6 diagonal values. Falling back to defaults.",
        parameter_name.c_str());
  }

  for (size_t i = 0; i < 6; ++i) {
    covariance[i * 6 + i] = values[i];
  }
...
void RangerROSMessenger::LoadParameters() {
  ...
  const auto odom_pose_covariance_diagonal =
      node_->declare_parameter<std::vector<double>>(
          "odom_pose_covariance_diagonal",
          {0.08, 0.08, 1e6, 1e6, 1e6, 0.35});
  const auto odom_twist_covariance_diagonal =
      node_->declare_parameter<std::vector<double>>(
          "odom_twist_covariance_diagonal",
          {0.03, 0.03, 1e6, 1e6, 1e6, 0.45});

  odom_pose_covariance_ = MakeDiagonalCovariance(
      odom_pose_covariance_diagonal,
      {0.08, 0.08, 1e6, 1e6, 1e6, 0.35},
      node_->get_logger(),
      "odom_pose_covariance_diagonal");
  odom_twist_covariance_ = MakeDiagonalCovariance(
      odom_twist_covariance_diagonal,
      {0.03, 0.03, 1e6, 1e6, 1e6, 0.45},
      node_->get_logger(),
      "odom_twist_covariance_diagonal");
...
void RangerROSMessenger::PublishStateToROS() {
  ...
  // Use the latest feedback motion mode for odometry integration so
  // turn-start and turn-stop transitions are not integrated with a stale mode.
...
void RangerROSMessenger::UpdateOdometry(double linear, double lateral,
                                        double angular, double angle, double dt) {
  const bool is_dual_ackermann =
      motion_mode_ == MotionState::MOTION_MODE_DUAL_ACKERMAN ||
      motion_mode_ == RangerInterface::MotionMode::kDualAckerman;
  const bool is_parallel =
      motion_mode_ == MotionState::MOTION_MODE_PARALLEL ||
      motion_mode_ == RangerInterface::MotionMode::kParallel;
  const bool is_side_slip =
      motion_mode_ == MotionState::MOTION_MODE_SIDE_SLIP ||
      motion_mode_ == RangerInterface::MotionMode::kSideSlip;
  const bool is_spinning =
      motion_mode_ == MotionState::MOTION_MODE_SPINNING ||
      motion_mode_ == RangerInterface::MotionMode::kSpinning;

  // update odometry calculations
  if (is_dual_ackermann) {
    ...
    position_y_ = x[1];
    theta_ = x[2];
  } else if (is_side_slip) {
    position_x_ += -linear * std::sin(theta_) * dt;
    position_y_ += linear * std::cos(theta_) * dt;
  } else if (is_spinning) {
  ...
  odom_msg.pose.covariance = odom_pose_covariance_;
  ...
  if (is_dual_ackermann) {
    ...
    odom_msg.twist.twist.linear.x = linear * std::cos(angle);
    odom_msg.twist.twist.linear.y = linear * std::sin(angle);
    odom_msg.twist.twist.angular.z = 0;
  } else if (is_side_slip) {
    odom_msg.twist.twist.linear.x = 0.0;
    odom_msg.twist.twist.linear.y = linear;
    odom_msg.twist.twist.angular.z = 0;
  } else if (is_spinning) {
  ...
  odom_msg.twist.covariance = odom_twist_covariance_;
